...
import lib.RandomControl as rnd_ctrl
import AutoSim
import pandas as pd
...
controller = rnd_ctrl.RandomController(m, d, motors)
...
with mujoco.viewer.launch_passive(m,d,show_left_ui=False,show_right_ui=False) as viewer:
    ...
    while viewer.is_running():
        ...
        # Get data for contact predictor
        ...
        torso_projected_grav = cdg.get_body_orientation(m, d, 'torso')
        head_projected_grav = cdg.get_body_orientation(m, d, 'head')
        ...
        if step_count % 5 == 0:
            ...
            current_obs = cdg.extract_observation_with_noise(actual_positions, 
                                                             target_positions, 
                                                             motor_torques, 
...
df.to_csv('training_data/random_data_noise2.csv', index=False)
print(f"Data saved to random_data_noise.csv with {len(df)} rows")